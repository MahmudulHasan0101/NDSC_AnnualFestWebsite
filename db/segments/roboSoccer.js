...
/**
 * Register a participant for Robo Soccer.
 *
 * Rules:
 ...
 */
async function registerRoboSoccer(userId, userName, email, teamName, partnerEmail, _unused) {
  ...
  try {
    ...
    if (trimmedPartner) {
      ...
      const [existingPartnerReg] = await conn.execute(
        'SELECT id FROM reg_robosoccer WHERE user_id = ? LIMIT 1',
        [partner.id]
      );
      if (existingPartnerReg.length) {
        await conn.rollback();
        const err = new Error(`${partner.full_name} is already registered for Robo Soccer.`);
      ...
      const [partnerRegRows] = await conn.execute(
        'SELECT * FROM reg_robosoccer WHERE user_id = ? LIMIT 1',
        [partner.id]
      );
...
async function getRoboSoccerReg(userId) {
...
module.exports = { registerRoboSoccer, getRoboSoccerReg };