...
const VALID_SB_CATEGORIES = ['Junior 6-10', 'Secondary 11-12'];

/**
 * Register a participant for Scrapbook.
 * Identical rules to Wall Magazine:
 *  - Optional partner (same institution if provided).
 *  - Team name required.
 *  - Transactional auto-enroll of partner.
 *
 * Error codes:
 ...
 */
async function registerScrapbook(userId, userName, email, teamName, category, partnerEmail, _unused) {
  if (!VALID_SB_CATEGORIES.includes(category)) throw new Error('Invalid category selection.');
  ...
  try {
    ...
    if (trimmedPartner) {
      ...
      const [existingPartnerReg] = await conn.execute(
        'SELECT id FROM reg_scrapbook WHERE user_id = ? LIMIT 1',
        [partner.id]
      );
      if (existingPartnerReg.length) {
        await conn.rollback();
        const err = new Error(`${partner.full_name} is already registered for Scrapbook.`);
      ...
      await conn.execute(
        `INSERT INTO reg_scrapbook
           (user_id, user_name, email, team_name, category, partner_email, partner_user_id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [partner.id, partner.full_name, partner.email, teamName.trim(), category, email, userId]
      );
      const [partnerRegRows] = await conn.execute(
        'SELECT * FROM reg_scrapbook WHERE user_id = ? LIMIT 1',
        [partner.id]
      );
    ...
    await conn.execute(
      `INSERT INTO reg_scrapbook
         (user_id, user_name, email, team_name, category, partner_email, partner_user_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [userId, userName, email, teamName.trim(), category, trimmedPartner, partnerUserId]
    );
...
async function getScrapbookReg(userId) {
...
module.exports = { registerScrapbook, getScrapbookReg };