...
const VALID_WM_CATEGORIES = ['Junior 6-10', 'Secondary 11-12'];

/**
 * Register a participant for Wall Magazine.
 *
 * Rules:
 *  - Partner email is OPTIONAL. If omitted, solo entry is allowed.
 *  - If provided, both participants must share the same institution.
 *  - Partner is auto-enrolled with the same team name and category.
 ...
 */
async function registerWallMagazine(userId, userName, email, teamName, category, partnerEmail, _unused) {
  if (!VALID_WM_CATEGORIES.includes(category)) throw new Error('Invalid category selection.');
  ...
  try {
    ...
    if (trimmedPartner) {
      ...
      const [existingPartnerReg] = await conn.execute(
        'SELECT id FROM reg_wallmagazine WHERE user_id = ? LIMIT 1',
        [partner.id]
      );
      if (existingPartnerReg.length) {
        await conn.rollback();
        const err = new Error(`${partner.full_name} is already registered for Wall Magazine.`);
      ...
      const [partnerRegRows] = await conn.execute(
        'SELECT * FROM reg_wallmagazine WHERE user_id = ? LIMIT 1',
        [partner.id]
      );
...
async function getWallMagazineReg(userId) {
...
module.exports = { registerWallMagazine, getWallMagazineReg };